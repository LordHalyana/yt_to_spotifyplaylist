...
def test_is_reasonable_match_partial_artist():
    assert matching.is_reasonable_match(
        "Daft", "One More Time", "Daft Punk", "One More Time"
    )


def test_is_reasonable_match_artist_mismatch():
    assert not matching.is_reasonable_match(
        "Daft Punk", "One More Time", "Justice", "One More Time"
    )


def test_is_reasonable_match_title_mismatch():
    assert not matching.is_reasonable_match(
        "Daft Punk", "One More Time", "Daft Punk", "Digital Love"
    )
