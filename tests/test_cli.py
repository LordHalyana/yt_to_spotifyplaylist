...
@mock.patch('yt2spotify.cli.load_config', return_value={'batch_size': 1, 'batch_delay': 1, 'max_retries': 1, 'backoff_factor': 1})
@mock.patch('yt2spotify.cli.sync_command')
def test_cli_main_sync_invokes_sync_command(mock_sync_command, mock_load_config, monkeypatch):
    # Simulate command-line arguments for the sync command
    test_args = [
        'prog', 'sync', 'https://youtube.com/playlist?list=PL123', 'SPOTIFY_PLAYLIST_ID',
        '--dry-run', '--no-progress', '--verbose', '--yt-api-key', 'FAKE_KEY', '--config', 'fake_config.toml'
    ]
