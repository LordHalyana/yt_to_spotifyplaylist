...
def test_sync_command_already_in_playlist(monkeypatch, tmp_path):
    ...
    with open(all_results_path, encoding="utf-8") as f:
        data = json.load(f)
    logger.debug(f"all_results.json (already_in_playlist): {data}")
    assert any(
        (d.get("track") == "one more time" and d.get("status") == "already_in_playlist")
        for d in data
    )
...
#     with open(all_results_path, encoding="utf-8") as f:
#         data = json.load(f)
#     logger.debug(f"all_results.json (added): {data}")
